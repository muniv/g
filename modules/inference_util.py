...
class InferenceModule(object):
    def __init__(self, model_name, max_model_len, gpu_memory_utilization=0.9):

        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        
        self.inference_module = LLM(model=model_name, tokenizer=model_name, tokenizer_mode='auto',
    ...
    def get_input_format(self, messages, prompt_text, replace_content: dict):
        for _key, _content in replace_content.items():
            prompt_text = prompt_text.replace(f'{_key}', _content)

        messages = [messages, {"role": "user", "content": prompt_text}]
    ...
    async def async_generate(self, module_input, sampling_params):
        async with self.semaphore:
            return await asyncio.to_thread(
                self.inference_module.generate,
                module_input,
    ...
    async def run_inference(self, module_input, involve_keys, temperature, top_p, generation_max_token, stop, max_retries=3):
        ...
        msg = f'Model Inference Max Retries Exceeded ({max_retries}) | {msg}'
        print(msg)

        return {"status": status, "msg": msg, "data": hypothesis}
    
    async def run_chat(self, module_input, temperature, top_p, generation_max_token, stop, max_retries=3):
        print(f'Inference Start!')
